...
```
brain (Claude or rules) ──proposes──▶ Policy gate ──allowed──▶ Toolbox ──▶ Razorpay ──webhook──▶ ledger
        │                                │ blocked                 │                         │
...
- **Graceful failure**, demonstrated and tested:
  ...
  - Claude API errors / timeouts / refusals / schema-invalid or out-of-policy output → falls back to the deterministic rules brain for that decision, recorded as `brain_fallback` with the cause. Nothing stalls.
...
```bash
uv sync                                   # Python 3.12+, installs anthropic, razorpay, fastapi
...
uv run pytest                             # 12 tests: policy, audit chain, webhooks, simulation
```

With Claude (optional): `export ANTHROPIC_API_KEY=…` then `uv run python -m baaki run --brain claude --demo-faults`. `--demo-faults` also blacks out the LLM on day 3 so you can watch the fallback happen. Model defaults to `claude-opus-5` at `low` effort with the system prompt cached; see `.env.example`.
...
4. **Brains** ([`brain.py`](baaki/brain.py)) — `RuleBrain` (deterministic playbook), `ClaudeBrain` (reads the reply and history, drafts the message, picks the action), `ResilientBrain` (Claude with rules fallback), plus the two baselines.
...
- The rules-brain run is what's in the tables above. The Claude brain path is fully wired and falls back cleanly, but I had no API key on this machine, so its numbers aren't reported here — run `--brain claude` to produce them.
...
baaki/
  ...
  brain.py        RuleBrain, ClaudeBrain, ResilientBrain, baselines, fault injection
